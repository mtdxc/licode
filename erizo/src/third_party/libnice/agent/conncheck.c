...
static gboolean priv_update_selected_pair (NiceAgent *agent, Component *component, CandidateCheckPair *pair)
{
  ...
  bool bChange = false;
  if(agent->controlling_mode)
    bChange = (pair->priority > component->selected_pair.priority);
  else
    bChange = (pair->local!=component->selected_pair.local || pair->remote!=component->selected_pair.remote);
  if(bChange){
    gchar laddr[46]; nice_address_to_string(&(pair->local->addr), &laddr);
    gchar raddr[46]; nice_address_to_string(&(pair->remote->addr), &raddr);
    guint lport = nice_address_get_port(&(pair->local->addr));
    guint rport = nice_address_get_port(&(pair->remote->addr));
    nice_debug ("[JANUS] Agent %p : changing SELECTED PAIR for component %u: %s:%s | %s:%d -> %s:%d"
        "(prio:%" G_GUINT64_FORMAT ").", agent, component->id, pair->local->foundation, pair->remote->foundation,
    	laddr,lport, raddr,rport, pair->priority);
...
static gboolean priv_schedule_triggered_check (NiceAgent *agent, Stream *stream, Component *component, NiceSocket *local_socket, NiceCandidate *remote_cand, gboolean use_candidate)
{
  ...
  for (i = stream->conncheck_list; i ; i = i->next) {
	...
	nice_debug ("Agent %p : Found a matching pair %p for triggered check.state %d", agent, p, p->state);
	...
	if (p->state == NICE_CHECK_WAITING ||
	  ...
	  if ((agent->compatibility == NICE_COMPATIBILITY_RFC5245 ||
	    ...
	    priv_conn_check_initiate (agent, p);
	} else if (p->state == NICE_CHECK_FAILED || use_candidate) {
