...
bool RtcpRrGenerator::handleRtpPacket(packetPtr packet) {
  ...
  if (ssrc_ != head->getSSRC()) {
    ELOG_DEBUG("message: handleRtpPacket ssrc not matched, ssrc: %u!=%u", ssrc_, head->getSSRC());
    return false;
  }
...
packetPtr RtcpRrGenerator::generateReceiverReport() {
  ...
  RtcpHeader* rtcp_head = (RtcpHeader*) packet_;
