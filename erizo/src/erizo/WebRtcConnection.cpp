...
WebRtcConnection::WebRtcConnection(std::shared_ptr<Worker> worker, std::shared_ptr<IOWorker> io_worker, 
  ...
  pipeline_{Pipeline::create()}, worker_{worker}, io_worker_{io_worker} {
  audioEnabled_ = videoEnabled_ = bundle_ = audio_muted_ = video_muted_ = pipeline_initialized_ = false;
  set_log_context(connection_id_.c_str());
  Info("constructor, stunserver: %s, stunPort: %d, minPort: %d, maxPort: %d",
      iceConfig.stun_server.c_str(), iceConfig.stun_port, iceConfig.min_port, iceConfig.max_port);
      ...
      if (remoteSdp_.hasVideo || bundle_) {
        ...
        if (videoTransport_.get() == nullptr) {
          ...
          videoTransport_.reset(new DtlsTransport(VIDEO_TYPE, "video", 
            connection_id_, bundle_, remoteSdp_.isRtcpMux, listener, iceConfig_ , false, worker_, io_worker_));
          videoTransport_->start();
        }
...
void WebRtcConnection::muteStream(bool mute_video, bool mute_audio) {
  Info("muteStream, mute_video: %u, mute_audio: %u", mute_video, mute_audio);
